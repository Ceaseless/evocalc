...
function sumTier(tier, fromLevel, toLevel) {
    let total = 0;
    for (let level = fromLevel; level <= toLevel; level += 1) {
        total += tierPoints[tier][level - 1];
    }
    return total;
}
...
function calculatePoints() {
    ...
    const startTierPriority = tierPriority[startTier];
    const endTierPriority = tierPriority[endTier];
    if (startLevel === NaN || endLevel === NaN || startLevel < 1 || endLevel < 1 || startLevel > 100 || endLevel > 100) {
        return;
    }
    if (endTierPriority < startTierPriority || (startTierPriority === endTierPriority && (endLevel < startLevel || startTier != endTier))) {
        result.textContent = "Current weapon higher than target.";
        return;
    }
    ...
    if (startTier === endTier) {
        gTotal = sumTier(startTier, startLevel + 1, endLevel);
    } else {
        // Finish start tier
        gTotal += sumTier(startTier, startLevel + 1, 100);

        // Intermediate tiers
        for (let state = 0; state < tierPoints.length; state++) {
            const tier = tierPriority[state];
            if (tier > startTierPriority && tier < endTierPriority) {
                gTotal += sumTier(state, 1, 100);
            }
        }

        // End tier
        gTotal += sumTier(endTier, 1, endLevel);
    }
    displayCosts(gTotal);
